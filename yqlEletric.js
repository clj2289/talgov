...
client.get(parcels, function (data, response) {
    ...
    for (var i = 0; i < obj.features.length; i++) {
        ...
        var taxidToPremisesId = {
        	prem_id_str: null,
            TAXID: obj.features[i].attributes.TAXID

        };

        console.log(obj.features[i].attributes.TAXID);
        ...
        new YQL.execp('select * from html where url="http://datamart.talgov.com/pls/dmart/account_search.matching_premises?zip_5_str=' + obj.features[i].attributes.ZIP1 + '&street_name_str=' + address + '&button_sw=Lookup%20Account" and xpath="//table"').then(function (response) {
            try {
                ...
                taxidToPremisesId.prem_id_str = x.premise_id_str;
               

                taxidToPremisesIds.push(taxidToPremisesId);
                ...
                new YQL.execp('select * from html where url="http://datamart.talgov.com/pls/dmart/dm_www_user.consumption_form.display_results?prem_id_str=' + prem_id_str + '&svc_type_str=E" and xpath="//table//table[1]/tr"').then(function (response) {
                    console.log("jere");
                    ...
                    for (var i = 0; i < response.query.results.tr.length; i++) {
					    ...
					    var outputFilename = 'taxidToPremisesIds.json';

					    fs.writeFileSync(outputFilename, JSON.stringify(taxidToPremisesIds));
